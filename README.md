...
Run through this checklist top to bottom on any new instance before expecting

`docker compose up -d` or Nginx to work correctly.
...
Pi-hole needs to bind host port 53 for DNS, which `systemd-resolved`'s stub

listener occupies by default on Ubuntu.
...
Nginx itself reads from `/etc/nginx/sites-available/` and `/etc/nginx/sites-enabled/`,

so both need to be symlinked back to the repo copies — never copy, always symlink,

so editing the file in the repo is editing the live config.
...
Get the VM's IP (bridged network mode required — NAT mode is not directly

reachable from the host without port forwarding):
...
Look for the address under your bridged adapter (e.g. `enp0s3`), typically in

your home LAN's range (`192.168.x.x`).



On **Windows**, edit `C:\Windows\System32\drivers\etc\hosts` (Notepad run as

Administrator, "All Files" filter to see the extensionless file):
...
Then open `https://pihole.local` and `https://audiobookshelf.local` directly

in a browser. Expect a self-signed certificate warning — this is expected and

correct behavior for a self-signed cert; click through ("Advanced" → "Proceed").
...
When adding a new reverse-proxied service, repeat the pattern from steps 5-9:

1. Write `nginx/sites-available/<service>.conf` in the repo (two server blocks:

   port 80 redirect + port 443 SSL, matching the existing configs as a template)

2. Symlink into `sites-available/` and `sites-enabled/`

3. Generate a self-signed cert with the correct CN

4. Add the domain to the VM's `/etc/hosts` **and** the Windows host's `hosts` file

5. `nginx -t && systemctl reload nginx`, then test
